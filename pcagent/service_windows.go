...
const (
	serviceName    = "AtomAirPCAgent"
	serviceDisplay = "Atom Air PC Power Agent"
	serviceDesc    = "Reports this PC's power state to the store gateway over MQTT, " +
		"so the room's air conditioner can be turned off automatically once every PC in it is off."
	stopTimeout = 10 * time.Second
)

// winService adapts Agent to the Windows service control manager.
...
func (w *winService) Execute(_ []string, r <-chan svc.ChangeRequest,
	...
	go func() { done <- NewAgent(w.cfg).Run(ctx) }()
	...
	for {
		...
		case err := <-done:
			if err != nil {
				slog.Error("pcagent stopped with an error", "err", err)
				return false, 1
			}
			return false, 0
		}
...
func installService(m *mgr.Mgr, cfg Config, args []string) error {
	...
	fmt.Printf("  pc_key    %s (add this to a room in the gateway's room_config.json)\n", cfg.PCKey)
	fmt.Printf("  reports   -> mqtt://%s:%d\n", cfg.MQTTHost, cfg.MQTTPort)
